    ...
    if (!e.meta || (e.meta && !e.meta.hideInMenu)) {
      if (saleStatus === "0") {
        if (
          ...
          if (hasChild(e) && showThisMenuEle(e, access))
            obg.submenu = getMenuByRouter(e.children, access);
  ...
  cloneData.forEach((e) => {
    keys.forEach((k) => {
      if (e.hasOwnProperty(k))
        e[k] = e[k] === null ? null : `${e[k].toLocaleString()}人`;
    });
  });
  return cloneData;
};
...
export const findCascadeLastLevel = (arr, child, callback) => {
  function traversal(a) {
    for (let i = 0; i < a.length; i++) {
      if ((a[i][child] && a[i][child].length) || a[i].itype === "floor") {
        //a[i].itype==='floor'  专为解决楼层下无实体时，全部按钮加在了楼层列
        traversal(a[i][child] || []);
      } else {
        callback(a);
        return;
...
export const getUnique = (arr) => {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    for (var k = 0; k < arr.length; k++) {
      if (k != i && arr[k].length == temp.length) {
...
export const getCompareDate = (originDate, compateType) => {
  if (Array.isArray(originDate)) {
    switch (compateType) {
      ...
      case "onChain": //环比
        const diff =
          Moment(originDate[0]).diff(Moment(originDate[1]), "days") - 1;
        return [
          Moment(originDate[0])
            .add(diff, "d")
            .format("YYYY-MM-DD"),
