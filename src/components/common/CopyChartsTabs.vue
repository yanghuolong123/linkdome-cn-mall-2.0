<template>
  <i-tabs v-model="current" ref="chartsTabs" v-if="isTypeChange">
    ...
    >
      <div :style="item === 'bar' ? '' : 'margin-top:20px;'" class="graphStyle">
        <vue-apex-charts
          ...
          :options="getOpt(item)"
          :series="
            (item === 'radialBar' ? radiabarSeries : cloneSeries()) || []
          "
      ...
      <custom-legend
        :data="lengendData"
        v-if="lengendData.length"
        :unit="$t(unit)"
      ></custom-legend>
      <div
        class="noDataStyle"
        v-if="isNoData"
        :class="'noDataStyle' + item"
      ></div>
    </tab-item>
...
export default {
      ...
      let column = this.megerSeries.map((e, index) => {
        if (index !== 0) {
          if (Array.isArray(this.tooltipUnit)) {
            return {
              title: e.name + " ( " + this.tooltipUnit[index - 1].name + " ) ",
              key: e.key,
            };
          } else {
            if (this.tooltipUnit) {
              return {
                title: this.$t(e.name) + " ( " + this.tooltipUnit + " ) ",
                key: e.key,
              };
            } else {
            ...
            if (this.megerSeries[cindex] && this.megerSeries[cindex].key) {
              if (!_.isNumber(this.megerSeries[cindex].key)) {
                if (this.megerSeries[cindex].key.match(/[A-Za-z]+/)) {
                  if (
                    ["RepeatPurchaseRate"].includes(
                      this.megerSeries[cindex].key.match(/[A-Za-z]+/)[0]
                    )
                  ) {
    ...
    radiabarSeries() {
      if (this.series.length) {
        ...
        return this.series.map((e) =>
          sumSeries ? _.round((e / sumSeries) * 100, 1) : 0
        );
      } else return [];
    },
    lengendData() {
  ...
  methods: {
    getOpt(type) {
      let tmlOptions = this.labels.data
        ? { ...opt[type], ...{ labels: this.labels.data } }
        : {
            ...opt[type],
            ...{
              xaxis: {
      ...
      if (tmlOptions.tooltip) {
        ...
        if (unit == "时间") {
          tmlOptions.xaxis.labels.formatter = (value) => {
            return initTimes(value);
          };
          tmlOptions.tooltip.y.formatter = (val) => {
      ...
      if (type === "bar") {
        ...
        let barOpt = {
          ...
          plotOptions: {
            bar: {
              columnWidth:
                categoriesLen < 5 ? (categoriesLen < 3 ? "10%" : "20%") : "70%",
              dataLabels: {
                position: "center",
                maxItems: 100,
        ...
        if (
          this.extraOptions &&
          this.extraOptions.plotOptions &&
          this.extraOptions.plotOptions.bar.horizontal
        ) {
        ...
        if (!optionsData.labels) {
          if (
            ...
            optionsData.xaxis.categories.length === 0
          ) {
