<template>
    <modal ref="modal"
        ...
        <Form :model="formData" label-position="right"
            ...
            <FormItem label="任务类型" prop="type_id">
                <el-radio v-model="formData.type_id" :label="0">分解任务</el-radio>
                <el-radio v-model="formData.type_id" :label="1">整体任务</el-radio>

            </FormItem>
            <FormItem label="执行日期" prop="date" v-if="formData.type_id===0">
                <DatePicker :value="formData.date" format="yyyy-MM-dd" @on-change="dateChange" type="daterange"
                            placement="bottom-end" style="width: 100%"></DatePicker>
            </FormItem>
            <FormItem label="执行时间" prop="hourtime" v-if="formData.type_id===0">
                <TimePicker v-model="formData.hourtime" confirm type="timerange" placement="bottom-end"
    ...
    methods: {
      ...
      addRecord () {
        ...
        let data = {
          ...
          date: this.formData.type_id===1?'':this.formData.date.toString(),
          time: this.formData.type_id===1?'':this.formData.hourtime.toString(),
        }
        if (this.isModify) {
