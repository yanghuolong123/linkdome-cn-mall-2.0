<template>
	<div>
		...
		<chart-box chartId="enter" :chart="enterChart" ref="chartEnter"
				   ...
				   :showSummary="true"
				   @toolClick="(chartName)=>{handletoolClick(chartName,'chartEnter',enterSelect,responseData)}"
			...
			<div class="flex-center quota">
				...
				<i-switch
						...
						class="ml-20 switch"
						size="large"
						@on-change="(val)=>{compareTypeChange(val,'enter')}" v-model="isHour">
		...
		<!--顾客客流量趋势-->
		<chart-box chartId="customer" :chart="customerEnterChart" ref="customerChartEnter"
							 :isDateCompare="oParams&&oParams.isDateCompare()"
							 :showSummary="true"
							 @toolClick="(chartName)=>{handletoolClick(chartName,'customerChartEnter',customerEnterSelect,customerResponseData)}"
							 class="common-card m-t-20 chart-1"
							 :toolList="customerToolList">
			<div class="flex-center quota">
				<span class="quota-label">{{ $t('fx.Data_indicators') }}</span>
				<Select v-model="customerEnterSelect" multiple :max-tag-count="1" @on-change="customerEnterSelectChange">
					<Option v-for="item in enterFlowList" :value="item.value" :key="item.value">{{ $t(item.name) }}</Option>
				</Select>
				<i-switch
					v-if="oParams && !oParams.isDateCompare() && !((oParams && oParams.isSingleDay())||(customerEnterSelect.length>1&&(oParams&&oParams.params.entitys.length>1)))"
					class="ml-20 switch"
					size="large"
					@on-change="(val)=>{compareTypeChange(val,'customer')}" v-model="c_IsHour">
					<span slot="open">{{ $t('小时') }}</span>
					<span slot="close">{{ $t('fx.day') }}</span>
				</i-switch>
			</div>
		...
		<!--店员客流量趋势-->
		<chart-box chartId="staff" :chart="staffEnterChart" ref="staffChartEnter"
							 :isDateCompare="oParams&&oParams.isDateCompare()"
							 :showSummary="true"
							 @toolClick="(chartName)=>{handletoolClick(chartName,'staffChartEnter',staffEnterSelect,staffResponseData)}"
							 class="common-card m-t-20 chart-1"
							 :toolList="staffToolList">
			<div class="flex-center quota">
				<span class="quota-label">{{ $t('fx.Data_indicators') }}</span>
				<Select v-model="staffEnterSelect" multiple :max-tag-count="1" @on-change="staffEnterSelectChange">
					<Option v-for="item in enterFlowList" :value="item.value" :key="item.value">{{ $t(item.name) }}</Option>
				</Select>
				<i-switch
					v-if="oParams && !oParams.isDateCompare() && !((oParams && oParams.isSingleDay())||(staffEnterSelect.length>1&&(oParams&&oParams.params.entitys.length>1)))"
					class="ml-20 switch"
					size="large"
					@on-change="(val)=>{compareTypeChange(val,'staff')}" v-model="s_IsHour">
					<span slot="open">{{ $t('小时') }}</span>
					<span slot="close">{{ $t('fx.day') }}</span>
				</i-switch>
			</div>
