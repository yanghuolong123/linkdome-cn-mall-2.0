<template>
	...
	<div>
		...
		<div v-if="type==2">
			<div style="position:relative">
				...
				<div class="topArea" v-bind:class="{ mohu: hasCover==3 }">
					...
					<!-- 商铺列表 -->
					<group-list v-if="addType === 3" :userLvl="userLvl" @imgConfig="imgConfig" @editStore="editStore" @delStore="delStore"
								@changeDoorway='changeDoorway'
		...
		<alert
				...
				:alertText='alertText'
		></alert>
		<imgconfig-modal ref="imgcofig" title="配置图片" :footerHide="true" @handleClose="graphType = 'line'" :width="950">
			<div class="img-config" @mousemove="handleMouseMove">
				<div class="part">
					<div class="img-container" style="margin-top: 0" ref="imgContainer"
						 @drop.stop.prevent="handleDropOnPannel($event)"
						 @dragover.stop.prevent>
						<img width="500" height="400" ref="img" :src="configImage" v-if="configImage">
						<img width="500" height="400" ref="img"
							 v-else
				...
				<div class="part">
					...
					<span v-else-if="addType===2">{{floorInfo[1] && floorInfo[1].name}}下属出入口店铺坐标配置</span>
					<span v-else>{{storeInfo[storeInfo.length-1].name}}店铺配置</span>
					<Select v-if="addType===3" v-model="graphType" :disabled="!isConfigDataSame" style="width:160px" @on-change="graphTypeChange">
						<Option  value="line" >多段线配置</Option>
						<Option  value="polygon" >区域配置</Option>
					</Select>
					<div class="flex-center">
		...
		<imgconfig-modal ref="preview" title="预览" :footerHide="true" :width="950">
			<div class="flex-column">
				<div ref="viewContainer" id="viewContainer">
					<img v-if="cameraImageUrl"  :src="cameraImageUrl">
					<img
						 v-else
						 src="../../assets/images/fixation_img/bg/placeholder.jpg">
...
<script>
  ...
  import easyFlowMixin from '@/mixin/jsplumb.js'
  export default {
    name: 'entity',
    ...
    computed: {
      ...
      configImage () {
        switch (this.addType) {
		  case 1:
            ...
            if (property) return property.map_url;
            break;
		  case 2:
      ...
      //配置图片的数据是否发生了变化
      isConfigDataSame () {
        return objEqual(this.configData, this.configOriginData)
      }
    },
    mounted () {
      this.getData()
    ...
    methods: {
      graphTypeChange(value){
        const floorId = this.floorInfo[1].id;
        const floor = deepFind(this.orgData,(item)=>{
        ...
        this.wayList = store[value] || [];
        this.way = this.wayList[0]&&this.wayList[0].id
        this.wayChange(this.way)
	  },
      previewClick(){
        let params = {};
        if(this.graphType === 'line'){
          params.line_id=this.currentWay.id
		}else {
          params.polygon_id=this.currentWay.id
		}
        getCameraImageUrl(params).then(res=>{
		  this.cameraImageUrl = res.data.data.screen_capture
        })
        this.jsPlumb && this.jsPlumb.reset();
        this.$refs.preview.showModal();
        this.$nextTick(()=>{
          this.formatGraphData([this.currentWay],this.graphType)
        })
          ...
          for (let j = 0; j < specificationLength; j++) {
            ...
            if (j < specificationLength - 1) {
              this.graphViewData.lineList.push({
                ...
                to: `${i}${j + 1}`,
                graphId: o.id,
                isLastLine: type === 'polygon' ? false : j === specificationLength - 2
              })
            }
            if (j === specificationLength - 1) {
              if (type === 'polygon') {//首尾相连
                this.graphViewData.lineList.push({
                  from: this.graphViewData.nodeList[this.graphViewData.nodeList.length - 1].id,
                  to: `${i}0`,
                  graphId: o.id,
                  isLastLine: false
                })
      ...
      handleSave () {
        ...
        switch (image_type) {
          ...
          case 'store':
            ...
            data[`${this.graphType}_id`] = this.currentWay.id;
            break
        }
        data.image_type = image_type
        data = _.merge(data, this.configData)
        configEntity(data).then(res => {
      ...
      handleReset () {
        ...
        if (symbol) {
          //本来就有坐标
          if (!isEmpty(this.configOriginData.position_x) && !isEmpty(this.configOriginData.position_y)) {
        ...
        } else if (!isEmpty(this.configOriginData.position_x) && !isEmpty(this.configOriginData.position_y)) {//本来有坐标，但是点了删除坐标时要恢复坐标
          const symbolNew = {
      ...
      wayChange (val) {
        ...
        if (way) {
          this.configData = {
            position_x: way.position_x,
            position_y: way.position_y,
      ...
      handleDropOnPannel (event) {
        ...
        const img = this.$refs.img
        if (event.target.className === 'symbol') {//如果移动距离过小，target落在symbol上
          ...
          symbol.left = left > 473 ? 473 : left < 0 ? 0 : left
          symbol.top = top > 365 ? 365 : top < 0 ? 0 : top
        } else {
        ...
        this.$nextTick(() => {
          switch (this.addType) {
			...
			case 3:
			  ...
			  this.storeImageUrl = store.map_url;
              this.wayList = store[this.graphType] || [];
      ...
      //点击放置坐标
      setClick (e) {
        if (this.hasSymbol) {//删除坐标
          const index = this.symbolList.findIndex(o => {
            return o.id === this.currentWay.id
          })
          this.symbolList.splice(index, 1)
          this.configData.position_x = ''
          this.configData.position_y = ''
        } else {//放置坐标
          this.addStatus = true
          const modalEl = document.getElementsByClassName('ivu-modal')[0]
          const imgContainer = this.$refs.imgContainer
      ...
      setCoordinate () {
        ...
        if (this.symbol.left < (width - 27) && this.symbol.left > 0 && this.symbol.top > 0 && this.symbol.top < (height - 35)) {
          //在范围内
      ...
      getSymbolList () {
        ...
        const wayList = this.wayList.filter(o => {
          return !isEmpty(o.position_x) && !isEmpty(o.position_y)
        })
        const img = this.$refs.img
