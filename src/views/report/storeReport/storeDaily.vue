  ...
  export default {
    ...
    methods: {
      downloadReport (type, time) {
        ...
        let url = pdfUrl + '/#/downloadStoreDayPdf?propertyId=' + this.propertyId + '&date=' + time + '&token=' + token + '&bzid=' + this.storeBzids+'&storeNames='+encodeURI(this.storeNames)+ '&date=' + time + '&token=' + token + '&pdfUrl=' + this.pdfBaseUrl
        this.$vs.loading()
