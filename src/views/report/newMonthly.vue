	...
	<div class="report-center" style="min-width: 1200px;">
		...
		<div class="pdf-text-box" v-if="showPDF" style="max-width:1200px">
			<div id="pdfDom">
				...
				<!-- 客流趋势 -->
				<report-chart :chartHeight='600' :clickData='clickData' title='客流趋势' :listTitle='trendTitle' page='3'
				...
				></report-ratio-table>
				<!-- 出入口 -->
				<report-chart :chartHeight='600' :clickData='clickData' title='出入口客流' page='5' :listTitle='gate10Title'
											:dataList='gateTop10'></report-chart>
				...
				<!-- 楼层 出入口 -->
				<report-chart-multi title='出入口客流' page='7' :listTitle='gateFloorTitle'
														:dataList='floorGateChartData'></report-chart-multi>
				
				<!-- 店铺 -->
				<report-chart :chartHeight='600' :clickData='clickData' title='店铺客流' page='8' :listTitle='shop10Title'
											:dataList='shop10Data'></report-chart>
				<!-- 楼层 店铺 -->
				<report-chart-multi title='店铺客流' page='9' :listTitle='floorShopTitle'
														:dataList='floorShopChartData'></report-chart-multi>
				<!-- 业态 店铺 -->
				<report-chart-multi title='店铺客流' page='10' :listTitle='shopcAtivitiesTitle'
														:dataList='formatShopChartData'></report-chart-multi>
				<!-- 热力图 -->
				<report-heat-map
					:key="'heatMap'+index"
				...
				<report-table title='店铺关联' :listTitle='orderlyTitle' :tableData='orderlyData'
											:page='`${11+allHeatMap.length}`'></report-table>
				<!-- 店铺关联 无序-->
				<report-table title='店铺关联' :listTitle='disorderTitle' :tableData='disorderData'
											:page='`${12+allHeatMap.length}`'></report-table>
				<!-- 停留时间 业态-->
				<report-chart :chartHeight='600' :clickData='clickData' :isRemark='false' title='停留时间'
											:page='`${13+allHeatMap.length}`' :listTitle='dwellTitle' :dataList='dwellChartData'
											chartType='dwell'></report-chart>
				<!-- 停留时间 业态下的商铺-->
				<report-chart-multi chartType='dwell' title='停留时间' :page='`${14+allHeatMap.length}`'
...
<script>
  ...
  export default {
      ...
      return {
        ...
        options3: {
          disabledDate (date) {
            ...
            return (moment(curDate).month() < moment(date).month() + 1) && (moment(date).year() >= moment(curDate).year())
          }
        },
        trendChartData: {
          option: {},
    ...
    mounted () {
      let newDate = moment(new Date()).subtract(1, 'months').format('YYYY-MM')
        ...
        data.forEach((o, index) => {
          ...
          o.list.forEach((gate,gIndex)=>{
            if(this.gateTableData[gIndex]){
              this.gateTableData[gIndex].currTotal = this.gateTableData[gIndex].currTotal+gate.current_num
          ...
          //排序取top10
          this.gateTableData = _.sortBy(this.gateTableData,'currTotal').reverse().splice(0,10);
