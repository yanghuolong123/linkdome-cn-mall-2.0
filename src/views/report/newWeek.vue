...
<div class="report-center" style="min-width: 1200px;">
  ...
  <div class="pdf-text-box" v-if="showPDF"  style="max-width:1200px">
    <div id="pdfDom">
      ...
      ></report-ratio-table>
      <!-- 出入口 -->
      <report-chart :chartHeight='600' :clickData='clickData'
                    title='出入口客流' page='4' :listTitle='gateTitle'  :dataList=gateChartData></report-chart>
      <!-- 出入口表格 -->
      <report-week-five
        title='出入口客流'
      ...
      <!-- 停留时间 业态-->
      <report-chart :chartHeight='600'  :clickData='clickData' :isRemark='false' title='停留时间' :page='`${12+allHeatMap.length}`'  :listTitle='dwellTitle' :dataList='dwellChartData' chartType='dwell'></report-chart>
      <!-- 停留时间 业态下的商铺-->
...
export default {
  ...
  methods: {
    ...
    dateTimeCount(value){
      var start = moment(value).startOf('isoweek').format('YYYY-MM-DD')//获取本周周一
      var end = moment(value).endOf('isoweek').format('YYYY-MM-DD')//获取本周周日
      ...
      let lastYearStart = lastStart + moment(value).startOf('isoweek').format('-MM-DD')
      let lastYearEnd = lastEnd + moment(value).endOf('isoweek').format('-MM-DD')
