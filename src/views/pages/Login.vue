<template>
    <div class="login-container" :class="themeClass">
        <!--国际化按钮-->
        <language-btn class="lang-btn"></language-btn>
        <div class="login">
            <img
            ...
            >
                <FormItem prop="username">
                    <input
                            ...
                            :placeholder="Text.holder.accountNum"
            ...
            <Button
                    class="login-btn"
                    :disabled="!validForm('formValidate')"
                    :loading="loading"
                    type="primary"
...
<script>
  ...
  import md5 from 'md5'
  import {capitalize,titleCase,mf} from '../../libs/util'
  ...
  export default {
    ...
    computed: {
      ...
      Text(){
        let text = {
          ...
          holder:{
            accountNum:this.$t('fn.inputHolder', [this.$t('accountNum')])
          },
          validate:{
            required_account:this.$t('fn.emptyNtc',[this.$t('account')]),
            required_password:this.$t('fn.emptyNtc',[this.$t('password')]),
          }
        }
        mf([
          text.label,
        ], (i) => titleCase(this.$t(i)));
        mf([
          text.button,
          text.validate,
          text.holder,
        ], (i) => capitalize(this.$t(i)));
        return text
      }
    },
    activated () {
    ...
    methods: {
      validForm(name){
        let flag;
        this.$refs[name] &&this.$refs[name].validate(valid=>{
          flag = valid
        })
        return flag
      },
