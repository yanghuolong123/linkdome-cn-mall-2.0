  ...
  methods: {
              ...
              if (this.systemSelect === 1) {
                ...
                if (data.roles_id == 1 || data.roles_id == 2) {
                  menuList.forEach((m) => {
                    ...
                    m.subpagesList.forEach((k) => { pages_privilege.push(k.id + '') })
                  })
                  ...
                  pages_privilege = data.pages_privilege ? data.pages_privilege.split(',') : []
                ...
                theUserPage.map((m) => { return isNaN(Number(m)) ? Number(m) : 0 })
                theUserPage.sort()
                // 根据权限处理用户跳转的第一个页面

                // 第一个页面路由下标
                let homePageIndex = Number(theUserPage[0]) ? Number(theUserPage[0]) : 0
                if (homePageIndex) {
    ...
    openBILarge () {
      this.getBiUrl().then(res => {
        ...
        window.location.href = 'https://' + this.Biurl + '/#/?homeUrl=' + this.homeUrl + '&user=' + userName + '&token=' + token + '&userSource=login'
      }).catch(err => {
        this.loading = false
        this.showHint(err)
      })
