      ...
      <template>
        <div class="flex justify-between items-center mr-10">
          <span class="whitespace-no-wrap mx-4 text-sm">{{
            $t("fx.Data_indicators")
          }}</span>
          <vs-select
            autocomplete
            :multiple="canSelectMulti"
            v-model="curretIndicator"
            :max-selected="2"
            id="chartSelect"
            @change="curretIndicatorChange"
          >
            <vs-select-item
...
export default {
  ...
  computed: {
    ...
    isChangeCategories() {
      ...
      let filterFootfall;
      if (this.canSelectMulti) {
        filterFootfall = this.curretIndicator.filter((e) =>
          ["enter", "occupancy"].includes(e)
        );
      } else {
    ...
    finnalSeries() {
      ...
      if (this.isChangeCategories) {
        // 需要计算出enter / occupancy 的和
        series.forEach((e) => {
          e.data = [_.sum(e.data)];
        });
      }
  ...
  methods: {
    ...
    getTrendData: _.debounce(function(params) {
      ...
      let type = time[0].split("-")[2] === time[1].split("-")[2];
      weatherTrend({
        time1,
      ...
      Promise.all(reqs)
        .then((res) => {
          ...
          Object.keys(indicatorData).forEach((indicatorKey, index) => {
            ...
            let series = Object.keys(indicatorRes).map((date) => {
              ...
              if (["CloseRate", "RepeatPurchaseRate"].includes(indicatorKey))
                seriesData = seriesData.map((e) => NP.times(e, 100)); // 复购率和成交率需要乘以100
              seriesData = seriesData.map((val) =>
                Number(val) < 0 ? 0 : parseInt(val)
              );
              return {
                name: time2
                  ? `${indicatorData[indicatorKey].name} ${date
                      .split(",")
                      .join(" - ")}`
                  : this.$t(indicatorData[indicatorKey].name),
            ...
            if (time2 && this.innerRange !== "1h") {
              ...
              xAxis = Array.from(
                { length: maxLen },
                (v, k) =>
                  `第${k + 1}${this.innerRange === "1day" ? "天" : "月"}`
              );
              // 给时间较短的填充 null
              series.forEach((e) => {
          ...
          _.forIn(tml, (val, key) => {
            ...
            if (series)
              val.total[`${series.key}`] = _.sum(series.data).toLocaleString();
          });
          this.chartData = tml;
          this.canshow = true;
        })
    ...
    refreshChart() {
      if (this.$refs.charts && this.$refs.charts.$children.length) {
        ...
        let charts = components.filter(
          (item) =>
            item.$options.name === "TabItem" &&
            item.$attrs.icon !== "biaoge-copy"
        );
        charts.forEach((e) =>
          this.$nextTick(() => {
