<template>
	<div class="mt-6 ranking-box">
		<div class="box-card bg-white">
			<chart-tabs :title="mallTitle" :xAxis="leftXaxis" :series="leftSeries" :type="['bar']"
						horizonta></chart-tabs>
		</div>
		<div class="box-card bg-white">
			<template v-if="chartType ==='bar'">
				<chart-tabs :title="formatTitle" :xAxis="rightXaxis" :series="rightValue" :type="['bar']"
							horizonta></chart-tabs>
			</template>
			<template v-else>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      mallTitle () {
        if (this.ismallMore) {
          return [`${this.orderName}排行 T O P 10`, `${this.orderName}排行`]
        } else {
          return [`${this.orderName}排行`]
        }
      },
      leftFormatedData () {
    ...
    methods: {
          ...
          if (e.business_type_id) { // 去重处理
            ...
            if (!hasitem) {
              ...
              uniqarr.forEach(item => {
                if (item.business_type_id === e.business_type_id) item.enter += e.enter
              })
            }
          } else {
      ...
      handleCompareRes (res) {
        ...
        this.$refs.pieCharts.updateOptions({ labels: pieYaxis })
      }
    }
  }
</script>
