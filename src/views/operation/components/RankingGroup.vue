...
export default {
  ...
  computed: {
    ...
    bussinessTypes() {
      return Object.keys(this.bussinessDictList).map((e) => ({
        text: this.bussinessDictList[e],
        value: Number(e),
      }));
    },
    withAllOptions() {
    ...
    topShopData() {
      ...
      // 如果是对象 需要对 source 排序,
      var sortedObj = Array.isArray(Object.values(source)[0])
        ? source
        : this.sortdObj(source);
      ...
      return {
        ...
        series: Object.keys(sortedObj).map((e) => {
          ...
          return {
            name,
            key: `data_${e}`,
            data: Object.values(sortedObj[e]).map((e) => {
              return e.data;
            }), // 因为排序后的数据是数组，所以需要判断
          };
        }),
        ...
        chart: {
          events: {
            dataPointSelection: (event, chartContext, config) => {
              // 需要用箭头函数访问到Vue实例，传统函数 this 为图表实例
  ...
  methods: {
    ...
    businessDataList() {
      ...
      if (this.bizIndicator == "enter")
        bizTypesTopReqs = getBizTopFootFall(bizParams, this);
      else if (this.bizIndicator == "dwell")
        bizTypesTopReqs = getIndustryDwell(bizParams, this);
      else bizTypesTopReqs = getTopBussinessType(bizParams);
    ...
    shopAndStoreDataList() {
      ...
      let currentPropertyId = this.$store.state.home.headerAction;
      let typeId = _.find(this.bussinessTypes, (bizType) => {
        return bizType.value == this.bussinessType;
      });
      if (typeId) {
        let topShopParam = {
          time1: this.time1,
        ...
        if (currentPropertyId != 0)
          topShopParam.property_id = currentPropertyId;
        else topShopParam.company_id = this.$store.state.user.companyId;
        if (this.time2) topShopParam.time2 = this.time2;

        // let url =
        //   this.shopIndicator == "enter"
        //     ? getShopTopFootFall(topShopParam)
        //     : getTopShop(topShopParam, this);
        let url = [];
      ...
      // let totalUrl =
      //   this.shopIndicator == "enter"
      //     ? getShopTopFootFall(shopParams)
      //     : getTopShop(shopParams, this);
      let totalUrl = [];
      if (this.shopIndicator == "enter")
        totalUrl = getShopTopFootFall(shopParams);
      else if (this.shopIndicator == "dwell")
        totalUrl = getShopDwell(shopParams);
      else totalUrl = getTopShop(shopParams);
      topShopOfAllBizType.push(totalUrl);
      Promise.all(
        [topShopOfAllBizType, topShopOfBizType].map((innerPromises) => {
    ...
    sortdObj(obj) {
      ...
      const orderVals = Object.entries(sumValsByKey).sort(
        (a, b) => b[1] - a[1]
      );
      let sortedObj = {};
    ...
    getDict() {
      ...
      getBussinessDict({ property_id: propertyId })
        .then((res) => {
