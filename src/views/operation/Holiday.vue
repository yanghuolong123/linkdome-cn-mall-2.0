<template>
    <div class="holidayAnaysis">
        <div class="box-card bg-white selector-container">
            ...
            <vs-select class="w-select" @change="yearChangeData(1)"  autocomplete v-model="selectYear" >
        ...
        <div  class="box-card bg-white selector-container">
          <!-- <p class="title">节假日活动对比分析</p> -->
            <div class="flex-center">
                <vs-select class="w-select" autocomplete @change="yearChangeData(2)"  v-model="compareYear1" >
                    <vs-select-item  :value="item.value" :text="item.key" v-for="(item,index) in years" />
                </vs-select>
                <vs-select  class="w-select m-l-20" autocomplete v-model="compareHoliday1" >
                    <vs-select-item  :value="item.value" :text="item.key" v-for="(item,index) in holidayActives1" />
                </vs-select>
                </div>
            <div class="flex-center">
                <vs-select class="w-select" autocomplete @change="yearChangeData(3)" v-model="compareYear2" >
                    <vs-select-item  :value="item.value" :text="item.key" v-for="(item,index) in years" />
                </vs-select>
    ...
    handleSearchCompare () {
      ...
      getDateCompare(params).then(function (res) {
        ...
        resData1.forEach(function (m, index) {
          ...
          obj.end = (resData2[index] || resData2[index] === 0) ? resData2[index].toLocaleString()  : ' '
          tableList2.push(obj)
        })
