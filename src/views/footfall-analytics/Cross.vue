  ...
  <div>
    ...
    <div class="cross_analysis_time">
      <div :key="index" v-for="(item, index) in timeList">
        ...
        <p class="cross_time_data">
          <span>{{ showTimeOne }}</span> {{ item.data }}
        </p>
        <p
          class="scale-data"
          v-if="selectTyepConfirm !== 0"
          v-bind:class="{ scaleAction: item.isActive }"
        >
    ...
    <div v-if="selectTyepConfirm == 0" class="cross_analysis">
      ...
      <div class="progress-title">
        <div
          class="progress-text"
          :key="index"
          v-for="(item, index) in maxEnterList"
        >
          <p>{{ $t(item.name) }}</p>
          <span>{{
            index == 0
      ...
      <vChart
        v-if="isGraph"
        style="width:115%!important;height:600px;"
        ref="graphChart"
        :options="polar"
      />
    </div>
    <div class="cross_analysis">
      ...
      <div style="height:415px;overflow: hidden;position:relative">
        ...
        <div
          class="cross-table-line"
          style="height: 400px;"
          v-bind:class="{ crossTableActive: iconIndex == 1 }"
        >
          <table-default
            :tableName="tableName"
...
export default {
  ...
  methods: {
    ...
    paramsPrepare() {
      ...
      this.iconIndex = 0;
      time1 =
        moment(this.crossDate[0]).format("YYYY-MM-DD") +
        "," +
      ...
      if (this.selectType === 0) {
        ...
        time2 =
          moment(this.crossDateTwo[0]).format("YYYY-MM-DD") +
          "," +
    ...
    crossDataList(time1, time2, from_bzid, to_bzid, range) {
      // 请求数据
      var that = this;
    ...
    crossTitleData(data) {
      ...
      that.timeList[0].data = _.sumBy(tpc, function(o) {
        return o.realFlow;
      }).toLocaleString();
      var lpf = data.compareRateLPF ? data.compareRateLPF : 0;
      that.timeList[0].contrast = NP.times(Number(lpf), 100);
      lpf >= 0
        ? (that.timeList[0].isActive = true)
        : (that.timeList[0].isActive = false);
      // 客流转化深度
      that.timeList[1].data =
        NP.times(Number(data.passengerConverRate), 100) + "%";
      var pcr = data.compareRatePCR ? data.compareRatePCR : 0;
      that.timeList[1].contrast = NP.times(Number(pcr), 100);
      pcr >= 0
        ? (that.timeList[1].isActive = true)
        : (that.timeList[1].isActive = false);
      // 转化客流量
      ...
      cpf >= 0
        ? (that.timeList[2].isActive = true)
        : (that.timeList[2].isActive = false);
    ...
    crossGraphData(data) {
      ...
      // 计算 转化率 位置
      var percent = Number(
        (that.maxEnterList[1].data / that.maxEnterList[0].data).toFixed(3)
      );
      if (_.isNaN(percent)) {
        that.percentData = 0;
        that.marginLeft = 0;
    ...
    lineData(data, range) {
      ...
      if (that.siteTraffic.series[1]) {
        let maxLength;
        if (
          that.siteTraffic.series[0].data.length >
          that.siteTraffic.series[1].data.length
        ) {
        ...
        maxLength.forEach((list, index) => {
          if (
            !that.siteTraffic.series[0].data[index] &&
            that.siteTraffic.series[0].data[index] !== 0
          ) {
            that.siteTraffic.series[0].data.push(0);
          }
          if (
            !that.siteTraffic.series[1].data[index] &&
            that.siteTraffic.series[1].data[index] !== 0
          ) {
    ...
    avgDuration(data, range) {
      ...
      that.$refs.lineChartAvg.updateOptions({
        xaxis: {
          categories: that.siteTrafficAvg.chartOptions.xaxis.categories,
        },
      });
      var time1 = moment(this.crossDate[0]).format("YYYY-MM-DD");
      var time2 = moment(this.crossDate[1]).format("YYYY-MM-DD");
    ...
    avgLineXaxisType(size, type) {
      ...
      that.$refs.lineChartAvg.updateOptions({
        xaxis: {
          offsetX: that.siteTrafficAvg.chartOptions.xaxis.labels.offsetX,
        },
      });
