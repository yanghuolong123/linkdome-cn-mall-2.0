  ...
  methods: {
    ...
    initTrend(res, xaxisCategories) {
      ...
      if (res.status == 200) {
        ...
        trendAndAvg.repeatNumber = formatNumber(
          _.sum(lineSeries[0].data) - _.sum(lineSeries[1].data)
        );
        let temps = res.data.data;
        if (temps.length) {
          trendAndAvg.avgTimesNumber = formatNumber(
            _.sumBy(res.data.data, "avgFrequencyVisits") / temps.length
          );
        } else {
    ...
    initCircle(resCircle) {
      ...
      if (resCircle.status == 200) {
        that.chartColumns = [
          { key: "name", title: this.$t("类型"), align: "center" },
          {
            key: "enter",
    ...
    async paramsPrepare(value) {
      ...
      if (value.compareType == "not") {
      ...
      } else if (["time", "onYear", "onChain"].includes(value.compareType)) {
        // 时间对比
        ...
        var ress = await Promise.all([getEntityFlow({ range, bzid })]);
        initRes.push(ress[0]);
        let type = innerRange == "Month" ? "Month" : "day";
        var diff1 = moment(value.date1Array[1]).diff(
          moment(value.date1Array[0]),
          type
        );
        var diff2 = moment(value.date2Array[1]).diff(
          moment(value.date2Array[0]),
          type
        );
        ...
        var name1,
          name2,
          label = value.entitys[0].label,
          time1 = value.date1Array,
          time2 = value.date2Array;
        ...
        trendAndAvg.trendLineSeries[0].data =
          trendAndAvg.trendLineSeries[1].data;
        trendAndAvg.trendLineSeries[1].name = name2;
        trendAndAvg.trendLineSeries[1].data =
          trendAndAvg2.trendLineSeries[1].data;
        ...
        xaxisCategories.forEach(function(m, index) {
          let obj = {};
          obj.name = m;
          obj.begin = trendAndAvg.trendLineSeries[0].data[index]
            ? trendAndAvg.trendLineSeries[0].data[index].toLocaleString()
            : " ";
          obj.end = trendAndAvg.trendLineSeries[1].data[index]
            ? trendAndAvg.trendLineSeries[1].data[index].toLocaleString()
            : " ";
          trendTable.push(obj);
        });
        trendAndAvg.trendTable = trendTable;
        trendAndAvg.avgLineSeries[0].name = name1;
        trendAndAvg.avgLineSeries[1] = _.cloneDeep(
          trendAndAvg.avgLineSeries[0]
        );
        ...
        xaxisCategories.forEach(function(m, index) {
          let obj = {};
          obj.name = m;
          obj.begin = trendAndAvg.avgLineSeries[0].data[index]
            ? trendAndAvg.avgLineSeries[0].data[index]
            : " ";
          obj.end = trendAndAvg.avgLineSeries[1].data[index]
            ? trendAndAvg.avgLineSeries[1].data[index]
            : " ";
          avgTable.push(obj);
        });
        ...
        if (trendAndAvg.effectiveNumbser >= trendAndAvg2.effectiveNumbser) {
          ...
          } else if (trendAndAvg2.effectiveNumbser == 0) {
            //eslint-disable-line
            that.effective.addPrecent = "-100%";
          } else if (trendAndAvg.effectiveNumbser == 0) {
            //eslint-disable-line
            that.effective.addPrecent = "100%";
          } else {
            var a = Math.round(
              (trendAndAvg2.effectiveNumbser / trendAndAvg.effectiveNumbser) *
                100
            );
          ...
          that.isUp1 = true;
          if (trendAndAvg2.effectiveNumbser == 0) {
            //eslint-disable-line
            that.effective.addPrecent = "-100%";
          } else if (trendAndAvg.effectiveNumbser == 0) {
            //eslint-disable-line
        ...
        if (trendAndAvg.repeatNumber >= trendAndAvg2.repeatNumber) {
          ...
          if (trendAndAvg.repeatNumber == trendAndAvg2.repeatNumber) {
            ...
            that.repeat.addPrecent =
              100 -
              Math.round(
                (trendAndAvg2.repeatNumber / trendAndAvg.repeatNumber) * 100
              ) +
              "%";
          }
        } else {
        ...
        if (trendAndAvg.avgTimesNumber >= trendAndAvg2.avgTimesNumber) {
          ...
          if (trendAndAvg.avgTimesNumber == trendAndAvg2.avgTimesNumber) {
            ...
            that.avgTimes.addPrecent =
              100 -
              Math.round(
                (trendAndAvg2.avgTimesNumber / trendAndAvg.avgTimesNumber) * 100
              ) +
              "%";
          }
        } else {
        ...
        var circleSeries = circleNames.map(function(m, index) {
          ...
          obj.name = m;
          obj.data = [
            resCircleData1[index] ? resCircleData1[index] : 0,
            resCircleData2[index] ? resCircleData2[index] : 0,
          ];
          return obj;
        });
