...
export default {
        ...
        this.chartData[this.curretIndicator] &&
          this.chartData[this.curretIndicator].series.forEach((o) => {
            if (this.curretIndicator === "enter") {
              ...
              obj[`${o.key}_${this.time1}`] = parseInt(
                _.sum(Object.values(o.data[0]))
              ).toLocaleString();
            }
          });
      }
    ...
    xAxisObj() {
      ...
      if (len == 0) {
        ...
        if (data.code == 200) {
          ...
          if (
            this.curretIndicator == "enter" ||
            this.curretIndicator == "occupancy"
          ) {
            dataValues.map((val) => {
            ...
            Object.keys(Object.values(dataValues[0])[0]).map((val) =>
              obj.data.push(val)
            );
          }
          return obj;
          ...
          if (
            ...
            dataKeys.map((list) => {
              ...
              if (time[0] == time[1]) {
                ...
                let listValue = Object.values(
                  Object.values(data.data[list])[0]
                );
                listValue.map((val) => {
  ...
  methods: {
    ...
    getTrendData: _.debounce(function(params) {
      ...
      Promise.all(reqs)
        .then((res) => {
          // enter occupancy SaleAmount SquaerMetre ClossRate UnitPrice
          let tml = {};
              ...
              return {
                name: time2
                  ? `${indicatorData[indicatorKey].name} ${date
                      .split(",")
                      .join(" - ")}`
                  : indicatorData[indicatorKey].name,
